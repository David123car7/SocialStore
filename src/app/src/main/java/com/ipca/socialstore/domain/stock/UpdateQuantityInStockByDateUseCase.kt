...
class UpdateQuantityInStockByDateUseCase @Inject constructor(private val stockRepository: StockRepository) {
    suspend operator fun invoke(itemId : Int, date: String, quantity : Int) : ResultWrapper<Int>{
        return stockRepository.updateQuantityInStockByDate(itemId,date,quantity)
    }
}